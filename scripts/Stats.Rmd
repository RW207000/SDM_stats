...
```{r : 17 ohp lab values women vs men per centre}
...
```{r : andro lab values women vs men per centre}
andro_22_23_sex_centre <- Nov22_Nov23_minimal_dataset %>%
  group_by(Centre.ID.x, Sex.at.birth.x) %>%
  summarise("n" = length(Androstenedione.value[!is.na(Androstenedione.value)]),
            "mean 17OHP value" = mean(Androstenedione.value, na.rm = T),
            "median 17OHP value" = median(Androstenedione.value, na.rm = T),
            "sd 17OHP value" = sd(Androstenedione.value, na.rm = T),
            "min 17OHP value" = min(Androstenedione.value, na.rm = T),
            "max 17OHP value" = max(Androstenedione.value, na.rm = T)) %>%
  mutate_if(is.numeric, ~round(.,2))
